...
def get_bbox(index, hdf5_data):
    ...
    for key in ['label', 'left', 'top', 'width', 'height']:
        ...
        values = [
            hdf5_data[attr[i].item()][0][0].astype(int)
            for i in range(len(attr))
        ] if len(attr) > 1 else [attr[0][0]]
        attrs[key] = values
    return attrs


if __name__ == "__main__":
    if (os.path.isdir('valid') is not True):
        print('create directory: valid/')
        os.mkdir('valid')
    else:
        print('already preprocess train and valid file!')
        exit(0)
    with h5py.File('train/digitStruct.mat') as hdf5_data:
        for i in range(len(hdf5_data['digitStruct']['name'])):
            ...
            if (i < 3000):
                shutil.copyfile('train/' + img_name, 'valid/' + img_name)
                os.remove('train/' + img_name)
            ...
            for idx in range(arr_l):
                ...
                s = str(label) + ' ' + str(x_center) + ' ' + str(
                    y_center) + ' ' + str(bbox_width) + ' ' + str(bbox_height)
                if idx != (arr_l - 1):
                    s += '\n'
                fp.write(s)
            fp.close()
